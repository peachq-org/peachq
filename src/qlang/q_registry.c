...
#include "ops/idxop.h"        /* .attr.* engine calls + ray_attr_is_sorted/ray_index_has (column attributes) */
...
static ray_t* qj_ktbl_merge(ray_t* x, ray_t* y, int mode);   /* joins wave */
...
static ray_t* qj_item(ray_t* x, int64_t i) {
    ray_t* ia = ray_i64(i);
    ray_t* e = ray_at_fn(x, ia);
    ray_release(ia);
    return e;
}
static ray_t* qj_gen_item(ray_t* x, int64_t i) {
...
static ray_t* qj_ktbl_merge(ray_t* x, ray_t* y, int mode);       /* fwd */
static int    qj_same_schema(ray_t* a, ray_t* b);                /* fwd */
 ...
 * single-home statics above (q_is_keyed_table / q_enkey / q_table_flatten) —
 * never redefined.  Refcounts: wrapper args BORROWED, results OWNED;
...
static ray_t* qj_table_gather_idx(ray_t* t, const int64_t* idx, int64_t n) {
    int64_t nc = ray_table_ncols(t);
    ray_t* out = ray_table_new(nc > 0 ? nc : 1);
    if (RAY_IS_ERR(out)) return out;
    for (int64_t c = 0; c < nc && !RAY_IS_ERR(out); c++) {
...
static ray_t* q_lj_wrap(ray_t* x, ray_t* y)  { return qj_lj_core(x, y, 0, 0); }
static ray_t* q_ljf_wrap(ray_t* x, ray_t* y) { return qj_lj_core(x, y, 1, 0); }
static ray_t* q_ij_wrap(ray_t* x, ray_t* y)  { return qj_lj_core(x, y, 0, 1); }
static ray_t* q_ijf_wrap(ray_t* x, ray_t* y) { return qj_lj_core(x, y, 1, 1); }
static ray_t* q_pj_wrap(ray_t* x, ray_t* y)  { return qj_lj_core(x, y, 2, 0); }
...
static ray_t* q_uj_wrap(ray_t* x, ray_t* y)  { return qj_uj_core(x, y, 0); }
static ray_t* q_ujf_wrap(ray_t* x, ray_t* y) { return qj_uj_core(x, y, 1); }
...
static ray_t* q_asof_wrap(ray_t* t, ray_t* d) {
