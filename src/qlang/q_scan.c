/* q_scan.c — THE single string->value scanner home (contract: q_scan.h).
...
#include "qlang/q_scan.h"
#include "qlang/q_registry.h"  /* q_days_from_civil, q_date_valid, q_ts_compose(_checked) */
#include "core/numparse.h"     /* ray_parse_f64 — the month float twin */
#include <string.h>

static int q_digit(char c) { return c >= '0' && c <= '9'; }

static int dig_run(const char *s, int p) {
    int n = 0;
    while (q_digit(s[p + n])) n++;
    return n;
}
...
int q_scan_temporal(const char* src, int* p, num_el* out, const char** err) {
     ...
     * than fall back to the float-strand mess. */
    {
        ...
        if (dig_run(src, q) == 4 && src[q + 4] == '.' &&
            ...
            if (!q_date_valid(y, mo, d)) { *err = "bad date"; return -1; }
            if (src[q + 10] == 'D') {
...
int q_date_scan(const char* p, size_t len,
...
int q_month_scan(const char* p, size_t len, int64_t* months) {
    int64_t y = 0, mo = 0;
    int ok = 0;
