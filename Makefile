...
# openq: the `.h` namespace constants — SAME codegen as dotq_gen.h but a
# distinct symbol (OPENQ_H_BOOTSTRAP) via the SYMBOL override. ALWAYS-ON: loaded
# at q_runtime_create right after dotq_gen.h, so q_runtime.c #includes both.
...
test: $(TARGET) $(LIB_OBJ) $(TEST_OBJ)
	...
	RAY_DFD=$${RAY_DFD:-0} RAYFORCE_CORES=$(TEST_CORES) timeout 900 ./$(TARGET).test || \
	  { rc=$$?; if [ $$rc -eq 124 ]; then \
	      echo "TEST TIMEOUT after 900s — suite ~150-250s idle but 555s observed under concurrent load (2026-07-21); with RAY_DFD=1 suspect the DFD spinlock stall (ARCHITECTURE.md); rerun make test"; \
	    fi; exit $$rc; }
...
#     latest.json + wasm manifest, rsync to peachq.org in contract order.
#     DRY-RUN unless GO=1. GitHub -> website is a PULL (this box holds the key).
q-release:
	VERSION="$(VERSION)" tools/q-release.sh

q-upload:
	GO="$(GO)" tools/q-upload.sh
